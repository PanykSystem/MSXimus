...
- Campaña v36k: los cinco dados murieron en el SDC (una excepción sobre `psg2`, que ya no existe; Gowin aborta).
- Campañas v36l, v36m (con `maxfan 50`) y v36n: 0 de 15, todos sin rutar. La causa no era la dieta sino el dado (3e87b28): el latido mínimo del `dbg_uart` comparaba con `==` y el placer daba la misma colocación para dados distintos (3761 y 3847 idénticos, 3797/3803/3877 idénticos): quince dados que eran unos ocho. Vuelve el `>=` de la telemetría completa. `tools/encadenar_campanas.ps1` tira campañas una tras otra (place_option 1 y 2 alternos) hasta un gate OK.
