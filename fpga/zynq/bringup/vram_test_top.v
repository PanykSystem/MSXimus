// ============================================================================
//  vram_test_top.v — v9968_axi_backend + cliente sintetico (vram_test) contra
//  el HP0 a 150 MHz. Tabla de resultados en VRAM_BASE + 0x3F0000, via el backend. Sin codigo ARM: el PS se inicializa con ps7_init.tcl desde
//  xsdb (relojes, MIO, DDR) y ps7_post_config libera FCLK_RESET0_N.
//
//  LEDs: fase en curso (0..6 en binario); 1111 = tabla escrita.
// ============================================================================
module vram_test_top #(parameter FCLK_MHZ = 150) (
    ...
    // ---- canales wv2 entre el cliente y el backend ----
    wire        a_req, a_we, b_req;
    wire [21:0] a_addr, b_addr;
    wire [31:0] a_wdata;
    wire [3:0]  a_wmask;
    ...
    v9968_axi_backend #(.VRAM_BASE(32'h1000_0000)) backend (
        .a_req(a_req), .a_we(a_we), .a_addr(a_addr), .a_wdata(a_wdata), .a_wmask(a_wmask),
        .a_dout(a_dout), .a_done(a_done),
        .b_req(b_req), .b_addr(b_addr), .b_dout(b_dout), .b_done(b_done),
        .clk_x1_out(), .ready(bk_ready), .diag(bk_diag), .dbg_ops(bk_ops), .recal_req(1'b0),
    ...
    vram_test #(.FCLK_MHZ(FCLK_MHZ), .N_WORDS(16384), .RES_OFF(22'h3F0000)) client (
        .clk(fclk0), .rst_n(frst0_n & bk_ready),
        .a_req(a_req), .a_we(a_we), .a_addr(a_addr), .a_wdata(a_wdata), .a_wmask(a_wmask),
        .a_dout(a_dout), .a_done(a_done),
        .b_req(b_req), .b_addr(b_addr), .b_dout(b_dout), .b_done(b_done),
        .led(led), .done(done)
